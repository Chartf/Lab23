...
LRESULT CALLBACK WndProc(HWND hwnd, UINT Message, WPARAM wParam, LPARAM lParam) {
	...
	switch(Message) {
		...
		case WM_CREATE: {
			...
			Input1 = CreateWindow("Edit","",WS_BORDER | WS_CHILD | WS_VISIBLE,30,50,170,20,hwnd,NULL,NULL,NULL);
			Input2 = CreateWindow("Edit","",WS_BORDER | WS_CHILD | WS_VISIBLE,30,80,170,20,hwnd,NULL,NULL,NULL);
			
			CreateWindow("STATIC","Please input two numbers",WS_VISIBLE | WS_CHILD,30,20,170,20,hwnd, NULL, NULL, NULL);
		...
		case WM_COMMAND: {
			if (LOWORD(wParam) >= 1 && LOWORD(wParam) <= 4) {
				...
				GetWindowText(Input1, input1, 256);
				GetWindowText(Input2, input2, 256);
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
	...
	wc.hbrBackground = (HBRUSH)(RGB(255, 255, 0));
